...
const HighlightSection = ({ highlightThreads, courseId, isHomePage }) => {
    return (
        <section>
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(18rem,1fr))] gap-5 mx-6 max-w-full">   
            {highlightThreads.map((thread) => (
              <Link to={`${isHomePage ? `thread/${thread.id}` : `/course/${courseId}/thread/${thread.id}`}`} style={{display: 'contents' }} key={thread.id}>  
                <HighlightCard key={thread.id} title={thread.title} taName={`${thread.author.name} ${thread.author.surname}`} year={thread.author.year} comments={thread.comments} /> 
              </Link>
            ))}   
          </div>    
        </section>   
