  ...
  return (
    <div className='relative grid grid-cols-[auto,auto] gap-x-6 gap-y-1 mb-1 h-fit text-white text-right ml-10'>
      ...
      {showButton && (
        <button
          ref={buttonRef}
          className="absolute top-[85%] left-[93%] z-10 px-2 py-2 w-fit text-sm text-white text-nowrap rounded-md border border-software-orange bg-eerie-black hover:bg-general-highlight"
          role="menuitem"
          onClick={() => { 
            localStorage.clear();
            navigate("/login");
