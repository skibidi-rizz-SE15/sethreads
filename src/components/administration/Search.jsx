...
function Search({ registeredCourses, onSelectYear, onSelectCourse, onSearch }) {
  return (
    <div className='flex flex-wrap justify-center mt-10 gap-4 mx-8 w-fit'>
      <select className='flex grow w-[15rem] h-10 text-white text-center bg-steadfast rounded-md focus:outline-none focus:ring-1 focus:ring-software-orange'>
      ...
      <select className='flex grow w-[30rem] h-10 text-white text-center bg-steadfast rounded-md focus:outline-none focus:ring-1 focus:ring-software-orange'>
      ...
      <button className='flex justify-center items-center grow bg-software-orange hover:bg-software-orange-hover text-white font-bold py-2 px-4 rounded' onClick={onSearch}><FaSearch /></button>
    </div>
  )
}
